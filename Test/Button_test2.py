...
class btn_frame(QPushButton):                                                            # 부모는 무조건 한명이어야 하나?
    ...
    def paintEvent(self, event):
        ...
        if self.type == 1:
            ...
            points = [QPoint(self.w/2,0), QPoint(0, self.h/2), QPoint(self.w/2, self.h), QPoint(self.w, self.h/2)]
...
if __name__ == '__main__':
    ...
    win_main = btn_frame(x=10, y=10, w=200, h=50,  type=4)
    # win_main = btn_frame(x=10, y=10, w=150, h=150, text='bibi', type=1)
    win_main.show()
    sys.exit(app.exec_())